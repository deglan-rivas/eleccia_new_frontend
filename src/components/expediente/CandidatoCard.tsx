      ...
      <div className="flex items-center space-x-4 mb-4">
        ...
        <div className="flex-shrink-0">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
            candidato.candidatoStatus === 'CUMPLE' ? 'bg-green-100 text-green-800' :
            candidato.candidatoStatus === 'ALERTA' ? 'bg-yellow-100 text-yellow-800' :
            'bg-red-100 text-red-800'
          }`}>
            {candidato.candidatoStatus === 'CUMPLE' ? (
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
              </svg>
            ) : candidato.candidatoStatus === 'ALERTA' ? (
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {candidato.candidatoStatus === 'CUMPLE' ? 'Cumple' : 
             candidato.candidatoStatus === 'ALERTA' ? 'Alerta' : 'No Cumple'}
          </span>
        </div>
      </div>                   
